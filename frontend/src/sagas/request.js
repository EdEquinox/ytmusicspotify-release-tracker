...
export function setupWorkers(count = DEFAULT_WORKERS_COUNT) {
  ...
  const workersFork = function* () {
    for (let i = 0; i < count; i++)
      workers.push(yield fork(requestWorker, requestChannel, responseChannel, 6, 10000))
  }
      ...
      try {
        ...
        if (error instanceof FetchError) {
          if (request.callCount < retryLimit) {
            yield delay(retryDelay * (request.callCount + 1))
        ...
        // Global pacing to avoid Spotify API burst/rate-limit spikes.
