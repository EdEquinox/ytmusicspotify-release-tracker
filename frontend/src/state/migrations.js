...
const migrations = {
  ...
  9: (state) => {
    const allGroups = Object.values(AlbumGroup)
    const currentGroups = state.settings.groups
    const hasAllGroupsSelected =
      currentGroups.length === allGroups.length &&
      allGroups.every((group) => currentGroups.includes(group))

    if (!hasAllGroupsSelected) return state
