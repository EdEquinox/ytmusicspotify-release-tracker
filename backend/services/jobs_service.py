...
from core.config import ARTISTS_FILE, RELEASES_FILE
from core.json_io import _read_json_list
from models.schemas import LocalFetchJob, ReleaseItem, ReleaseSyncJob
...
def _run_release_sync_job(job_id: str, start_dt: datetime, end_dt: datetime) -> None:
    try:
        ...
        for row in artists:
            tid = _tidal_numeric_id_for_artist(row)
            if not tid:
                continue
            tracked = _tracked_artist_id_for_releases(row)
            if not tracked:
                continue
            tasks.append((tid, tracked, str(row.get("name", "")).strip()))
        total_artists = len(tasks)
        ...
        if total_artists == 0:
            ...
            _persist_last_releases_fetch_end_date(end_dt.date().isoformat())
            return

        session = load_tidal_session()
        if session is None:
            raise RuntimeError(
                "Sessão Tidal em falta. Abre a app, vai a Releases e completa o login Tidal antes do sync."
            )
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                processed += 1
                progress = int((processed / max(len(tasks), 1)) * 100)
                _update_release_job(
...
def _run_local_fetch_job(
    ...
    try:
        ...
        if session is None:
            raise RuntimeError(
                "Sessão Tidal em falta. Na página Releases, abre o link e completa o login Tidal antes do fetch."
            )
        ...
        for index, (tidal_id, tracked_id, name) in enumerate(tasks):
            fetched = _fetch_artist_releases_tidal(
                session,
                tidal_id,
                tracked_id,
                name,
            ...
            if index < len(tasks) - 1 and spacing_ms > 0:
