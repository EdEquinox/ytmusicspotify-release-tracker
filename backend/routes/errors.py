...
from core.config import CSV_RELEASES_FILE, ERRORS_FILE
from core.json_io import _read_json_list, _write_json_list
from models.schemas import SyncErrorCreate, SyncErrorItem, SyncErrorLinksUpdate
...
def resolve_error(error_id: str) -> dict[str, str | bool]:
    ...
    # Erros do worker principal vêm sempre de uma linha em csv_releases.json (release_id = id da fila).
    # Ao resolver, retirar essa linha para o item não voltar a ser processado.
    csv_removed = False
    rid = str(removed.get("release_id") or "").strip()
    if rid:
